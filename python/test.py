...
def fetch_external():
    ...
    for interval in intervals:
        ...
        index += 1
        response = requests.get(url_MDA)
        if response.status_code == 200:
            datos_MDA = response.json()["Resultados"][0]["Valores"]
            filtered_data = [item for item in datos_MDA if fecha_ini <= item['fecha'] <= fecha_fin]
            all_data.extend(filtered_data)
        else:
            print(f"Error fetching from {url_MDA}: {response.status_code}")
    
    return jsonify({"Resultados": [{"Valores": all_data}]})
